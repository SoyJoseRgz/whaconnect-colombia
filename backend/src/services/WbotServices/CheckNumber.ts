import { WASocket } from "@whiskeysockets/baileys";
import AppError from "../../errors/AppError";
import GetDefaultWhatsApp from "../../helpers/GetDefaultWhatsApp";
import { getWbot } from "../../libs/wbot";
import Whatsapp from "../../models/Whatsapp";
import logger from "../../utils/logger";
import { Session } from "../../libs/wbot";
import { ENABLE_LID_DEBUG } from "../../config/debug";

interface IOnWhatsapp {
  jid: string;
  exists: boolean;
  lid: string;
}

const toJid = (num: string) =>
  num.includes("@") ? num : `${num}@s.whatsapp.net`;

const checker = async (number: string, wbot: Session): Promise<IOnWhatsapp> => {
  const result = await wbot.onWhatsApp(toJid(number));

  if (!result) {
    logger.error({ number }, "Failed to check number on whatsapp");
    throw new AppError("ERR_INVALID_NUMBER", 400);
  }

  if (!result?.[0]?.exists) {
    throw new AppError("ERR_CHECK_NUMBER", 404);
  }

  const lid = (result as any).lid ?? null;

  const response = {
    jid: result[0]?.jid,
    exists: true,
    lid
  };

  if (ENABLE_LID_DEBUG) {
    logger.info(
      `[LID-DEBUG] Retornando contato: { jid: '${response.jid}', exists: true, lid: '${lid}' }`
    );
  }

  return response;
};

const CheckContactNumber = async (
  number: string,
  companyId: number,
  isGroup: boolean = false,
  userId?: number,
  whatsapp?: Whatsapp | null
): Promise<IOnWhatsapp> => {
  if (ENABLE_LID_DEBUG) {
    logger.info(
      `[LID-DEBUG] Buscando contato: number=${number}, companyId=${companyId}, lid=`
    );
  }

  const whatsappList =
    whatsapp || (await GetDefaultWhatsApp(companyId, userId));

  const wbot = getWbot(whatsappList.id);

  if (isGroup) {
    if (ENABLE_LID_DEBUG) {
      logger.info(`[LID-DEBUG] Processando grupo: ${number}`);
    }
    try {
      const groupJid = number.includes('@g.us') ? number : `${number}@g.us`;
      const meta = await wbot.groupMetadata(groupJid);
      if (ENABLE_LID_DEBUG) {
        logger.info(
          `[LID-DEBUG] Retornando contato: { jid: '${meta.id}', exists: true, lid: 'null' }`
        );
      }
      return { jid: meta.id, exists: true, lid: null };
    } catch (error) {
      logger.error({ number, error }, "Failed to get group metadata");
      throw new AppError("ERR_GROUP_NOT_FOUND", 404);
    }
  }

  if (whatsappList.channel === "whatsapp_oficial") {
    return { jid: toJid(number), exists: true, lid: null };
  }

  return checker(number, wbot);
};

export default CheckContactNumber;
