...
const handleMessage = async (
  ...
  try {
    ...
    const tck = await FindOrCreateTicketService(
      contact,
      whatsapp,
      unreadMessages,
      companyId,
    ...
    await provider(tck, msg, companyId, contact, wbot as WASocket);
    ...
    if (
      ...
      // Obtener mensaje personalizado de audio desde configuración
      const audioMessage = settings?.audioMessage || 
        `🤖 *Asistente Virtual*
...
Gracias por tu comprensión 😊`;
    ...
    if (
      ...
      (settings?.asaas &&
        ...
        //@ts-ignore
        (ticket?.queue?.name !== "2ª Via de Boleto" || ticket?.queue?.name !== "2 Via de Boleto") || (!settings?.asaas || settings?.asaas.trim() === "")
