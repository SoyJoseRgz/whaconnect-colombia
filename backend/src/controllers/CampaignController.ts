...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const processedRecurrenceData = {
      ...
      executionCount: 0,
      nextScheduledAt: !isRecurring && scheduledAt ? new Date(scheduledAt) : null,
      lastExecutedAt: null
    };
    ...
    if (campaign.isRecurring) {
      ...
      // Asegurar que la campaña se recargue para obtener el nextScheduledAt actualizado
...
export const update = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const processedRecurrenceData = {
      ...
      recurrenceEndDate: (isRecurring && recurrenceEndDate) ? new Date(recurrenceEndDate) : null,
      maxExecutions: (isRecurring && maxExecutions) ? maxExecutions : null,
      // Añadir campos que faltaban para mantener la consistencia
      executionCount: 0, // Reiniciar al actualizar
      nextScheduledAt: !isRecurring && scheduledAt ? new Date(scheduledAt) : null,
      lastExecutedAt: null
    };
    ...
    if (campaign.isRecurring) {
      ...
      // Para campañas no recurrentes, asegurar que el objeto devuelto esté actualizado
      await campaign.reload();
    }
